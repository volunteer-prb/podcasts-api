...
export class RestService {
  ...
  private includes(values: string[]|undefined): {} {
    ...
    return values.map(s => 'include_' + s).map(v => {
      let tmp: any = {}
      tmp[v] = true
      return tmp
  ...
  get(method: string, params: any = {}, options: RestOptions|undefined = undefined): Observable<any> {
  ...
  request(httpMethod: 'POST'|'GET'|'PUT'|'DELETE'|'UPLOAD',
          method: string,
          params: any,
          contentType: string|null='application/json; charset=UTF-8'): Observable<any> {
    if (!method.startsWith('/')) {
      method = '/' + method;
    }
    ...
    return this.http.request(httpMethod, url, options).pipe(
        map((value: any) => {
          ...
          if (value.data === undefined) {
            this.logger.error('Rest return no data', value, 'method', httpMethod, method, 'with params', params);
